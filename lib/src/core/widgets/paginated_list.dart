...
class _EasyPaginationState<T, E> extends State<EasyPagination<T, E>> {
    ...
    try {
      ...
      if(e is ServerException){
        if(e is UnauthorizedException){
          context.read<UserCubit>().handleLoggingOut(e);
          errorMsg = e.message;
        }
  ...
  Future<T> _callApi(Future<T> Function(int currentPage) asyncCall)async{
    ...
    switch(connectivityResult){
      case ConnectivityResult.none:
        throw PaginationNetworkError(LocaleKeys.checkInternet);
      default:
      ...
      itemBuilder: (context, index) {
        if (index < newItems.length) {
          ...
          if(widget.showNoDataAlert){
            ...
            Text(LocaleKeys.noMoreRestaurants, textAlign: TextAlign.center, style: const TextStyle(color: Colors.grey)) :
            _loadingWidget;
          }else{
  ...
  Widget _gridView() {
    return GridView.count(
      ...
      children: List.generate(
        widget.showNoDataAlert? newItems.length + 1 :
        status == AsyncCallStatus.loading? newItems.length + 1 : newItems.length, (index)  {
        if (index < newItems.length) {
          return widget.itemBuilder(newItems, index);
        } else {
          if(widget.showNoDataAlert){
  ...
  Widget get _loadingWidget{
    switch(widget.loadingBuilder){
      ...
      default:
        return Center(child: widget.loadingBuilder!);
    }
  }
  ...
  Widget get _buildErrorWidget{
    if(newItems.isNotEmpty){
      if(status == AsyncCallStatus.networkError){
        MessageUtils.showSimpleToast(msg: LocaleKeys.checkInternet, color: Colors.red);
      }else{
